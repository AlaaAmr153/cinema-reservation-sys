document.addEventListener('DOMContentLoaded', function () {
    // Fetch seat data when a showtime is selected
    const showtimeInput = document.getElementById('showtime');
    const layout = document.getElementById('seat-layout');

    if (showtimeInput) {
        // Add a change event to fetch seats for selected showtime
        showtimeInput.addEventListener('change', function () {
            const movieId = document.getElementById('cinema').value; // Assuming the movie ID is stored here
            const showtimeId = showtimeInput.value;

            fetch(`/seats?movie_id=${movieId}&showtime_id=${showtimeId}`)
    ...
    function renderSeats(seats) {
        layout.innerHTML = ''; // Clear existing layout

        seats.forEach(seat => {
            const seatElement = document.createElement('div');
            seatElement.classList.add('seat');
            seatElement.dataset.seatId = seat.id;
            seatElement.classList.add(seat.is_occupied ? 'occupied' : 'available');
            seatElement.addEventListener('click', toggleSeatSelection);

            layout.appendChild(seatElement);
        });
    }
    ...
    function toggleSeatSelection(event) {
        ...
        document.getElementById('seats').value = selectedSeatIds.join(',');
    }
});
