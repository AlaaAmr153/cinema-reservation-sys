    ...
    <div class="flex items-start justify-start p-12">
        <div class="w-full max-w-[550px] ">
            <form method="POST" action="{{route('showtimes.update',$showtime->id)}}">
                ...
                <input type="hidden" name="movie_id" id="movie_id" value="{{ old('movie_id', $showtime->movie_id) }}">
                <div class="mb-5">
                    ...
                    <div class="relative rounded-md border border-[#e0e0e0] font-medium text-[#6B7280]">
                        <div class="selectbox-header" id="selectboxHeader">
                            <img id="selectedImage" src="{{ asset($showtime->movie->poster) }}" alt="Selected Image"
                                class="selectbox-img">
                            <span id="selectedText">{{$showtime->movie->title}}</span>
                        </div>
                        <ul id="optionsList" class="selectbox-options">
                            @foreach ($movies as $movie )
                            <li class="selectbox-option" data-value="{{$movie->id}}" data-image="{{old('movie_id',asset($movie->poster)) }}" data-text="{{$movie->title}}">
                                <img src="{{ asset($movie->poster) }}" alt="{{$movie->title}}" class="option-img">
                ...
                <div class="mb-5">
                    ...
                    class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-black focus:shadow-md mb-3">
                        ...
                        <option value="{{$cinema->id}}" {{ $cinema->id == old('cinema_id', $showtime->cinema_id) ? 'selected' : '' }}>
                            {{$cinema->cinema_name}}
                        </option>
                        @endforeach
                </select>
            ...
            <div class="mb-5">
                ...
                class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-black focus:shadow-md mb-3">
                    ...
                    <option value="{{$screen->id}}" {{$screen->id == old('screen_id', $showtime->screen_id) ? 'selected' : ''}}>
                        {{ $screen->screen_code }}
                    </option>
                    @endforeach
            </select>
                ...
                {{-- <div class="mb-5">
                    ...
                    class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-black focus:shadow-md mb-3">
                        ...
                        @foreach ($screens as $screen) --}}
                        {{-- <option value="{{$id}}">{{$cinema_name}}</option> --}}
                        {{-- @endforeach
                </select>
                    ...
                    <div class="w-full">
                        <div class="mb-5">
                            ...
                            <input type="date" name="date" id="date" value="{{old('date',$showtime->show_date)}}"
                    ...
                    <div class="w-full">
                        <div class="mb-5">
                            ...
                            <input type="time" name="time" id="time" value="{{old('time',date('H:i', strtotime($showtime->show_time)))}}"
                                class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-black focus:shadow-md" />
